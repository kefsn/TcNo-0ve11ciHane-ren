...
public enum UsStatesEnum {
    ...
    -Aplikasyonlar Stand-Alone(tek başına ayakta kalma) olmalıdır.
     -Sabit data isimleri büyük harfle yazılır.ALABAMA gibi
    ...
    private final String stateName;//Enum'lar private yapsak iyi olur ama final olmak zorundadır.
    ...
    //State kisaltmasini verince state ismini veren method..
    public static String getStateNameByUsingAbbreviaton(String abbreviation){
        return Arrays.stream(values())
                .filter(usState -> usState.getAbbreviation().equals(abbreviation))
                .findFirst().get().getStateName();
    }

    //State ismini verince state'in kisaltmasini veren metod..
