...
*/
    ...
    public static void dogrucevapsayisi(char[][] answers, char[] keys) {
        ...
        for (int i = 0; i <answers.length ; i++) {// 0'dan öğrenci sayısına kadar döngüye aldık.
            ...
            int sayac = 0; //Sayacı burda oluşturmamızın sebebi, bir sonraki öğrenciye geçerken doğru cevap sayısını sıfırlasın istiyoruz

            for (int j = 0; j <answers[i].length ; j++) {//0'dan öğrencini cevap sayısına kadar döngüye aldık

                if (answers[i][j]==keys[j]){//Öğrenciniz cevabı, cevap anahtarı ile eşit mi?Kontrol ediyoruz.
                    sayac++;
                }
            }
            System.out.println(i+1 +" nolu öğrencinin cevap sayıs : " + sayac);
